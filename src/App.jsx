// T11 SPORTS v5 — Gestão Completa de Loja de Camisas de Time
// PADRÃO DEFINITIVO: Este arquivo é a base para todas as próximas versões
// Design: Cards com hover, bordas coloridas, números vibrantes, kanban
// Módulos: Dashboard | Estoque | Pedidos | Gestão (Kanban) | Custo | Caixa | Tarefas | Fornecedor
...
const KANBAN = [
  ...
  { key:"Pedido Feito", label:"ENTREGAR",  icon:"🚚", color:"#2563eb" },
...
const DB0 = {
  produtos:[], pedidos:[], caixa:[], tarefas:[], pedidosFornecedor:[],
...
const LBL = {fontSize:11,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.5px",display:"block",marginBottom:5};
const TH  = {textAlign:"left",padding:"9px 12px",fontSize:11,color:"#6b7280",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",borderBottom:"1px solid #e5e7eb",whiteSpace:"nowrap",background:"#fafafa"};
const TD  = {padding:"10px 12px",borderBottom:"1px solid #f5f5f5",fontSize:13,color:"#374151",verticalAlign:"middle"};
...
function Btn({children,v="primary",onClick,style:sx,disabled}) {
  ...
  return (
    <button onClick={onClick} disabled={disabled} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{padding:"9px 18px",borderRadius:8,cursor:disabled?"not-allowed":"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:6,transition:"all 0.15s",transform:hov&&!disabled?"translateY(-1px)":"none",boxShadow:hov&&!disabled?"0 4px 12px rgba(0,0,0,0.12)":"none",opacity:disabled?0.6:1,...V[v],...sx}}>
      {children}
    </button>
  );
}
...
function Badge({status}) {
  ...
  return <span style={{background:c.bg,color:c.fg,border:`1px solid ${c.bd}`,padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700,whiteSpace:"nowrap"}}>{status}</span>;
}
...
function HRow({children,onClick}) {
  ...
  return <tr onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick} style={{background:h?"#f9fafb":"#fff",transition:"background 0.12s",cursor:onClick?"pointer":"default"}}>{children}</tr>;
}
...
function HCard({children,style:sx,color,onClick}) {
  ...
  return (
    <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick}
      style={{background:"#fff",border:"1px solid #e5e7eb",borderLeft:color?`4px solid ${color}`:"1px solid #e5e7eb",borderRadius:10,padding:"16px 18px",transition:"all 0.18s",transform:h?"translateY(-2px)":"none",boxShadow:h?"0 8px 24px rgba(0,0,0,0.1)":"0 1px 3px rgba(0,0,0,0.04)",cursor:onClick?"pointer":"default",...sx}}>
      {children}
    </div>
  );
}
...
function KPI({label,value,sub,color,dark}) {
  ...
  return (
    <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
      style={{background:dark?"#111":"#fff",border:dark?"none":"1px solid #e5e7eb",borderRadius:10,padding:"16px 18px",transition:"all 0.18s",transform:h?"translateY(-2px)":"none",boxShadow:h?"0 8px 24px rgba(0,0,0,0.12)":dark?"0 2px 8px rgba(0,0,0,0.3)":"0 1px 3px rgba(0,0,0,0.04)",cursor:"default"}}>
      <div style={{fontSize:11,color:dark?"rgba(255,255,255,0.4)":"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:8}}>{label}</div>
    ...
    <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
      {options.map(({k,l})=>(
        <button key={k} onClick={()=>onChange(k)}
          style={{padding:"7px 14px",borderRadius:20,border:`1px solid ${value===k?"#111":"#d1d5db"}`,background:value===k?"#111":"#fff",color:value===k?"#fff":"#374151",cursor:"pointer",fontSize:12,fontWeight:value===k?700:400,fontFamily:"inherit",transition:"all 0.15s"}}>
          {l}
        </button>
      ))}
    </div>
...
function Modal({title,onClose,children,wide}) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,padding:28,width:wide?680:520,maxWidth:"100%",maxHeight:"90vh",overflowY:"auto",boxShadow:"0 25px 60px rgba(0,0,0,0.2)"}}>
...
function ModalBtns({onClose,onSave,label="Salvar"}) {
  return (
...
function InfoBox({items}) {
  return (
    <div style={{background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:10,padding:"12px 16px",marginBottom:14,display:"flex",gap:20,flexWrap:"wrap"}}>
...
function Empty({msg,icon="📭"}) {
  return <div style={{textAlign:"center",padding:"40px 20px",color:"#9ca3af"}}><div style={{fontSize:32,marginBottom:10}}>{icon}</div><div style={{fontSize:13}}>{msg}</div></div>;
}

function AlertBar({type,children}) {
  const styles={
    warning:{bg:"#fffbeb",border:"#f59e0b",text:"#92400e"},
    error:  {bg:"#fef2f2",border:"#dc2626",text:"#7f1d1d"},
    success:{bg:"#f0fdf4",border:"#16a34a",text:"#14532d"},
    info:   {bg:"#eff6ff",border:"#2563eb",text:"#1e3a5f"},
  };
  const s=styles[type]||styles.info;
  return (
    <div style={{background:s.bg,border:`1px solid ${s.border}`,borderLeft:`4px solid ${s.border}`,borderRadius:10,padding:"10px 16px",marginBottom:16,fontSize:13,color:s.text,display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
      {children}
    </div>
  );
}
...
function Login({onLogin}) {
  ...
  const go=()=>{setErr("");setLoad(true);setTimeout(()=>{u===AUTH.usuario&&p===AUTH.senha?onLogin():(setErr("Usuário ou senha incorretos."),setLoad(false));},500);};
  return (
    <div style={{minHeight:"100vh",background:"#111",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',-apple-system,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"44px 40px 36px",width:380,boxShadow:"0 30px 80px rgba(0,0,0,0.5)"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:52,marginBottom:10}}>⚽</div>
          <div style={{fontSize:26,fontWeight:900,color:"#111",letterSpacing:"-1px"}}>T11 Sports</div>
          <div style={{fontSize:13,color:"#9ca3af",marginTop:4}}>Gestão de Loja de Camisas</div>
        </div>
        <div style={{marginBottom:14}}><label style={LBL}>Usuário</label><Inp value={u} onChange={e=>setU(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} autoFocus placeholder="Seu usuário"/></div>
        <div style={{marginBottom:20}}><label style={LBL}>Senha</label><Inp type="password" value={p} onChange={e=>setP(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Sua senha"/></div>
        {err&&<div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:8,padding:"10px 14px",fontSize:13,color:"#dc2626",marginBottom:16,textAlign:"center"}}>{err}</div>}
        <Btn onClick={go} disabled={load} sx={{width:"100%",justifyContent:"center",padding:13,fontSize:15,borderRadius:10}}>{load?"Entrando...":"Entrar →"}</Btn>
        <div style={{textAlign:"center",marginTop:20,fontSize:11,color:"#d1d5db"}}>T11 Sports © {new Date().getFullYear()}</div>
      </div>
    </div>
  );
}
...
function FormPedido({inicial,produtos,onSave,onClose}) {
  const vz={data:hoje(),cliente:"",time:"",uniforme:"Uniforme 1",tamanho:"M",qtd:1,precoVenda:0,valorRecebido:0,custoProduto:0,custoTaxa:0,status:"A Fazer",obs:""};
  const [f,setF]=useState(inicial?{...vz,...inicial}:vz);
  const s=(k,v)=>setF(p=>({...p,[k]:v})); const n=(k,v)=>s(k,parseFloat(v)||0);
  const fill=time=>{ const pd=produtos.find(x=>x.time===time); if(pd) setF(x=>({...x,time,custoProduto:pd.custoProduto,custoTaxa:pd.custoTaxa,precoVenda:pd.precoVenda,tamanho:pd.tamanho})); else s("time",time); };
  const v=fmt((f.precoVenda||0)*(f.qtd||1)); const c=fmt(((f.custoProduto||0)+(f.custoTaxa||0))*(f.qtd||1));
  const l=fmt(v-c); const sb=fmt(v-(f.valorRecebido||0)); const mg=v>0?fmt((l/v)*100):0;
  ...
  return (
    <>
      ...
      <ModalBtns onClose={onClose} onSave={()=>{if(!f.cliente.trim())return alert("Informe o cliente.");if(!f.time.trim())return alert("Informe a camisa.");onSave(f);}}/>
    </>
  );
}
...
function FormProduto({inicial,onSave,onClose}) {
  ...
  const [f,setF]=useState(inicial?{...vz,...inicial}:vz);
  const s=(k,v)=>setF(p=>({...p,[k]:v})); const n=(k,v)=>s(k,parseFloat(v)||0);
  const ct=fmt((f.custoProduto||0)+(f.custoTaxa||0)); const mg=f.precoVenda>0?fmt(((f.precoVenda-ct)/f.precoVenda)*100):0;
  return (
    <>
      <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
        ...
        <Field label="Qtd Mínima (alerta)" half><Inp type="number" min="0" value={f.qtdMin} onChange={e=>n("qtdMin",e.target.value)}/></Field>
      ...
      <InfoBox items={[
        ...
        {label:"Margem",value:pct(mg),color:mg>=30?"#16a34a":mg>=15?"#ca8a04":"#dc2626"},
        {label:"Lucro Unit.",value:brl(fmt(f.precoVenda-ct)),color:f.precoVenda-ct>=0?"#16a34a":"#dc2626"},
      ]}/>
      <ModalBtns onClose={onClose} onSave={()=>{if(!f.time.trim())return alert("Informe o time.");onSave(f);}}/>
    </>
  );
}
...
function PageDashboard({db,setDb}) {
  ...
  const meses=useMemo(()=>{const s=new Set(db.pedidos.map(p=>p.data?.slice(0,7)).filter(Boolean));s.add(mes());return[...s].sort().reverse();},[db.pedidos]);
  ...
  const luc=fmt(fat-cus); const marg=fat>0?fmt((luc/fat)*100):0;
  const receb=pm.reduce((a,p)=>a+(p.valorRecebido||0),0); const pend=fmt(fat-receb);
  ...
  const salvarMeta=()=>{ setDb(prev=>({...prev,meta:metaTemp})); setEditMeta(false); };

  // Metas ativas (só mostra se > 0)
  const metasAtivas=[
    {label:"Pedidos",atual:pm.length,meta:db.meta.pedidos,fmtFn:v=>v,color:"#2563eb",icon:"🛒"},
    {label:"Posts",atual:0,meta:db.meta.posts||0,fmtFn:v=>v,color:"#7c3aed",icon:"📱"},
    {label:"Futebol",atual:0,meta:db.meta.futebol||0,fmtFn:v=>v,color:"#f97316",icon:"⚽"},
    {label:"Receita",atual:fat,meta:db.meta.receita,fmtFn:brl,color:"#16a34a",icon:"💰"},
    {label:"Lucro",atual:luc,meta:db.meta.lucro,fmtFn:brl,color:luc>=0?"#16a34a":"#dc2626",icon:"📈"},
  ].filter(m=>m.meta>0);

  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      {/* Hero banner */}
      <div style={{background:"linear-gradient(135deg,#111 60%,#1a1a2e 100%)",borderRadius:16,padding:"24px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 4px 24px rgba(0,0,0,0.18)"}}>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{fontSize:28,lineHeight:1}}>⚽</div>
            <div style={{fontSize:24,fontWeight:900,color:"#fff",letterSpacing:"-0.5px"}}>Olá, Fabrício!</div>
          </div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.4)",textTransform:"capitalize"}}>{dataStr}</div>
          {tarefasHj>0&&(
            <div style={{marginTop:10,display:"inline-flex",alignItems:"center",gap:6,background:"#f97316",borderRadius:20,padding:"5px 14px",fontSize:12,color:"#fff",fontWeight:700}}>
              📋 {tarefasHj} tarefa{tarefasHj>1?"s":""} para hoje
            </div>
          )}
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:44,fontWeight:900,color:"#fff",letterSpacing:"-3px",lineHeight:1}}>{hora}</div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.3)",marginTop:4}}>{new Date().toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric"})}</div>
        </div>
      </div>
      ...
      {/* KPIs operacionais — 4 cards de status */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        {[
          {label:"Pedidos a Fazer",icon:"📦",value:produzir,color:"#f97316",accent:"#fff7ed"},
          {label:"Em Transporte",  icon:"✈️", value:emTransp, color:"#2563eb",accent:"#eff6ff"},
          {label:"Entregue",       icon:"✅", value:entregue, color:"#16a34a",accent:"#f0fdf4"},
          {label:"Atrasados",      icon:"🚨", value:atrasados,color:"#dc2626",accent:"#fef2f2"},
        ].map(({label,icon,value,color,accent})=>{
          const [h,setH]=useState(false);
          return (
            <div key={label} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
              style={{background:"#fff",border:`1px solid #e5e7eb`,borderTop:`4px solid ${color}`,borderRadius:12,padding:"18px 20px",transition:"all 0.18s",transform:h?"translateY(-3px)":"none",boxShadow:h?"0 10px 28px rgba(0,0,0,0.1)":"0 1px 4px rgba(0,0,0,0.05)",cursor:"default"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div>
                  <div style={{fontSize:11,fontWeight:700,color:"#9ca3af",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:8}}>{label}</div>
                  <div style={{fontSize:36,fontWeight:900,color,lineHeight:1}}>{value}</div>
                </div>
                <div style={{width:44,height:44,borderRadius:10,background:accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{icon}</div>
              </div>
            </div>
          );
        })}
      ...
      <Section title="📊 Desempenho Mensal" action={
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <select value={mesSel} onChange={e=>setMesSel(e.target.value)} style={{...INP,width:150,padding:"6px 10px"}}>
            {meses.map(m=><option key={m} value={m}>{m}</option>)}
          </select>
        </div>
      }>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
          <KPI label="Pedidos"      value={pm.length}  dark={true}/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginTop:10}}>
          ...
          <KPI label="Ticket Médio" value={brl(pm.length>0?fmt(fat/pm.length):0)}/>
        </div>
      </Section>
      ...
      <Section title="🎯 Metas" action={
        ...
        {metasAtivas.length===0?(
          ...
          <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(metasAtivas.length,3)},1fr)`,gap:16}}>
            {metasAtivas.map(({label,atual,meta,fmtFn,color,icon})=>{
              const p=meta>0?Math.min(100,(atual/meta)*100):0;
              const [h,setH]=useState(false);
              return (
                <div key={label} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
                  style={{background:"#f9fafb",borderRadius:10,padding:"14px 16px",border:"1px solid #e5e7eb",transition:"all 0.15s",transform:h?"translateY(-1px)":"none",boxShadow:h?"0 4px 12px rgba(0,0,0,0.08)":"none"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      <span style={{fontSize:16}}>{icon}</span>
                      <span style={{fontWeight:700,fontSize:14,color:"#111"}}>{label}</span>
                    </div>
                    <span style={{fontWeight:900,fontSize:15,color}}>{fmtFn(atual)} <span style={{fontWeight:400,color:"#9ca3af",fontSize:12}}>/ {fmtFn(meta)}</span></span>
                  </div>
                  <Prog value={Math.max(0,atual)} max={meta} color={color}/>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#9ca3af",marginTop:4}}>
                    <span>{p.toFixed(0)}% concluído</span>
                    <span style={{color:p>=100?"#16a34a":"#9ca3af",fontWeight:700}}>{p>=100?"✅ Meta atingida!":""}</span>
                  </div>
                </div>
              );
            })}
          ...
          <>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
              {topVendas.map(([nome,qtd],i)=>(
                <div key={nome} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:i<topVendas.length-1?"1px solid #f5f5f5":"none"}}>
                  <div style={{width:28,height:28,borderRadius:"50%",background:i===0?"#111":"#f3f4f6",color:i===0?"#fff":"#6b7280",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,flexShrink:0}}>{i+1}</div>
                  <div style={{flex:1}}>
            ...
            <AlertBar type="success">💡 <strong>Sugestão IA:</strong> Priorize reposição de <strong>{topVendas[0]?.[0]}</strong> no próximo pedido ao fornecedor.</AlertBar>
          </>
        )}
      </Section>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {[...db.pedidos].reverse().slice(0,8).map(p=>{
                  const v=fmt((p.precoVenda||0)*(p.qtd||1)); const sb=fmt(v-(p.valorRecebido||0));
                  return (
                    <HRow key={p.id}>
      ...
      {editMeta&&(
        <Modal title="Editar Metas" onClose={()=>setEditMeta(false)}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
            ...
            <Field label="📱 Meta de Posts (redes sociais)" half><Inp type="number" min="0" value={metaTemp.posts||0} onChange={e=>setMetaTemp(m=>({...m,posts:parseInt(e.target.value)||0}))}/></Field>
            <Field label="⚽ Meta de Futebol (jogos/eventos)" half><Inp type="number" min="0" value={metaTemp.futebol||0} onChange={e=>setMetaTemp(m=>({...m,futebol:parseInt(e.target.value)||0}))}/></Field>
          </div>
          <ModalBtns onClose={()=>setEditMeta(false)} onSave={salvarMeta} label="Salvar Metas"/>
        </Modal>
      )}
    </div>
  );
...
function PageEstoque({db,onAdd,onEdit,onDelete}) {
  ...
  const filtrados=db.produtos.filter(p=>(!busca||(p.time||p.nome||"").toLowerCase().includes(busca.toLowerCase())||p.tamanho?.toLowerCase().includes(busca.toLowerCase()))&&(ft==="Todos"||(p.time||p.nome)===ft));
  ...
  return (
    <div>
      {baixos.length>0&&<AlertBar type="error"><span style={{fontWeight:800}}>⚠ Estoque baixo:</span>{baixos.map(p=><span key={p.id} style={{background:"#fecaca",borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700}}>{p.time} {p.uniforme} {p.tamanho} ({p.qtd}un)</span>)}</AlertBar>}
      {sugestoes.length>0&&<AlertBar type="success">💡 <strong>IA — Repor:</strong> {sugestoes.map(([k,v])=>`${k} (${v}vnd)`).join(" · ")}</AlertBar>}

      <div style={{display:"flex",gap:12,marginBottom:16}}>
        <HCard style={{flex:1}}>
          <div style={{fontSize:11,color:"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4,display:"flex",alignItems:"center",gap:6}}>📦 TOTAL PEÇAS</div>
          <div style={{fontSize:28,fontWeight:900,color:"#111"}}>{total}</div>
        </HCard>
        <HCard style={{flex:1}} color="#ca8a04">
          <div style={{fontSize:11,color:"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4,display:"flex",alignItems:"center",gap:6}}>⚠ ESTOQUE BAIXO</div>
          <div style={{fontSize:28,fontWeight:900,color:baixos.length>0?"#ca8a04":"#16a34a"}}>{baixos.length}</div>
        </HCard>
        <HCard style={{flex:1}}>
          <div style={{fontSize:11,color:"#9ca3af",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:4,display:"flex",alignItems:"center",gap:6}}>💰 VALOR EM ESTOQUE</div>
          <div style={{fontSize:22,fontWeight:900,color:"#111"}}>{brl(valor)}</div>
        </HCard>
      </div>
      ...
      <Section>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            <FilterTabs options={times.map(t=>({k:t,l:t}))} value={ft} onChange={setFt}/>
        ...
        {filtrados.length===0?<Empty msg="Nenhum produto encontrado." icon="📦"/>:(
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              ...
              <tbody>
                {filtrados.map(p=>{
                  const ct=fmt((p.custoProduto||0)+(p.custoTaxa||0));
                  return (
                    <HRow key={p.id}>
...
function PagePedidos({db,onAdd,onEdit,onDelete,onUpdateMeta}) {
  const [filtro,setFiltro]=useState("mes"); const [busca,setBusca]=useState(""); const [mm,setMm]=useState(false); const [mt,setMt]=useState({...db.meta});
  const m=mes(); const hj=hoje(); const sw=semIni();
  const mp=mesPrev();
  ...
  const fat=pm.reduce((a,p)=>a+(p.precoVenda||0)*(p.qtd||1),0);
  const cus=pm.reduce((a,p)=>a+((p.custoProduto||0)+(p.custoTaxa||0))*(p.qtd||1),0);
  const luc=fmt(fat-cus); const rec=pm.reduce((a,p)=>a+(p.valorRecebido||0),0);
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:16}}>
        <KPI label="Pedidos Mês" value={pm.length} dark icon="🛒"/>
        <KPI label="Faturamento" value={brl(fat)} color="#16a34a" icon="💰"/>
        <KPI label="Custo Mês" value={brl(cus)} color="#dc2626" icon="💸"/>
        <KPI label="Lucro Líquido" value={brl(luc)} color={luc>=0?"#16a34a":"#dc2626"} icon="📈"/>
        <KPI label="A Receber" value={brl(fmt(fat-rec))} color={fat-rec>0?"#ca8a04":"#16a34a"} icon="⏳"/>
      </div>

      {/* Meta */}
      <Section title={`🎯 Meta de ${m}`}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
          {[{l:"Pedidos",a:pm.length,m:db.meta.pedidos,f:v=>v},{l:"Receita",a:fat,m:db.meta.receita,f:brl},{l:"Lucro",a:luc,m:db.meta.lucro,f:brl}].map(({l,a,m,f})=>{
      ...
      <Section action={
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <FilterTabs options={FILTROS} value={filtro} onChange={setFiltro}/>
        ...
        {filtrados.length===0?<Empty msg="Nenhum pedido encontrado." icon="🛒"/>:(
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>{["Data","ID","Produto","QTD","Valor","Taxa/Custo","Lucro","Status",""].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead>
              <tbody>
                {[...filtrados].reverse().map(p=>{
                  const v=fmt((p.precoVenda||0)*(p.qtd||1)); const c=fmt(((p.custoProduto||0)+(p.custoTaxa||0))*(p.qtd||1));
                  const l=fmt(v-c); const mg=v>0?fmt((l/v)*100):0; const sb=fmt(v-(p.valorRecebido||0));
                  return (
                    <HRow key={p.id}>
                      <td style={{...TD,color:"#9ca3af"}}>{p.data}</td>
      ...
      {mm&&<Modal title="Editar Meta Mensal" onClose={()=>setMm(false)}>
        ...
        <ModalBtns onClose={()=>setMm(false)} onSave={()=>{onUpdateMeta(mt);setMm(false);}} label="Salvar Meta"/>
      </Modal>}
    </div>
  );
}
...
function PageGestao({db,onEdit,onAdd}) {
  ...
  const m=mes(); const hj=hoje(); const sw=semIni(); const mp=mesPrev();
  const FILTROS=[{k:"hoje",l:"Hoje"},{k:"semana",l:"Esta semana"},{k:"mes",l:"Este mês"},{k:"mes_ant",l:"Mês passado"},{k:"todos",l:"Todos"}];
  const filtrados=db.pedidos.filter(p=>{ if(filtro==="hoje")return p.data===hj; if(filtro==="semana")return p.data>=sw; if(filtro==="mes_ant")return p.data?.startsWith(mp); if(filtro==="todos")return true; return p.data?.startsWith(m); });
  return (
    <div>
      ...
      <div style={{marginBottom:16}}><FilterTabs options={FILTROS} value={filtro} onChange={setFiltro}/></div>
...
function PageCusto({db,setDb}) {
  ...
  const lucLiq=fmt(rec-cusProd-cusTaxa-totDesp);
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
        <div><div style={{fontSize:22,fontWeight:800,color:"#111"}}>Custo / Lucro</div><div style={{fontSize:13,color:"#9ca3af"}}>Despesas e resultado do mês</div></div>
        <Btn onClick={()=>setModalD(true)}>+ Despesa</Btn>
      </div>
      <div style={{marginBottom:16}}><FilterTabs options={FILTROS} value={filtro} onChange={setFiltro}/></div>
      <Section>
        <div style={{fontWeight:800,fontSize:13,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:16}}>RESUMO — {filtro.toUpperCase()}</div>
      ...
      {modalD&&<Modal title="Nova Despesa" onClose={()=>setModalD(false)}>
        <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
          ...
          <Field label="Descrição"><Inp value={fd.descricao} onChange={e=>setFd(p=>({...p,descricao:e.target.value}))} placeholder="ex: 21 unid. Bahia, Taxa..." autoFocus/></Field>
        ...
        <ModalBtns onClose={()=>setModalD(false)} onSave={salvD}/>
      </Modal>}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {modal&&<Modal title="Novo Lançamento" onClose={()=>setModal(false)}>
        ...
        <ModalBtns onClose={()=>setModal(false)} onSave={salv}/>
      </Modal>}
    </div>
  );
}
...
function PageTarefas({db,setDb}) {
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:16}}>
        <KPI label="Hoje" value={hjCnt} color={hjCnt>0?"#dc2626":"#16a34a"} icon="📅"/>
        <KPI label="Pendentes" value={pCnt} color={pCnt>0?"#ca8a04":"#16a34a"} icon="⏳"/>
        <KPI label="Concluídas" value={fCnt} color="#16a34a" icon="✅"/>
      </div>
      ...
      <Section action={<div style={{display:"flex",gap:8,alignItems:"center"}}><FilterTabs options={ABAS} value={aba} onChange={setAba}/><Btn onClick={()=>setModal(true)}>+ Tarefa</Btn></div>}>
  ...
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:16}}>
        {ST_FORN.map(st=><KPI key={st} label={st} value={grupos[st].length} color={st==="Chegou - Vendido"?"#16a34a":"#111"} icon={st==="Lista de Espera"?"📋":st==="Pedido Feito"?"📤":st==="Chegou - Estoque"?"📦":"✅"}/>)}
      </div>
      ...
      {ST_FORN.map(st=>grupos[st].length===0?null:(
        <Section key={st} title={<span style={{display:"flex",alignItems:"center",gap:10}}><Badge status={st}/><span style={{fontSize:13,color:"#9ca3af"}}>{grupos[st].length} item{grupos[st].length>1?"s":""}</span></span>}>
          <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>{["Data","Cliente","Camisa","Tam.","Qtd","Venda","Custo","Obs.","Mover",""].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead>
            <tbody>{grupos[st].map(p=>(
              <HRow key={p.id}>
                <td style={{...TD,color:"#9ca3af"}}>{p.data}</td>
                <td style={{...TD,fontWeight:700,color:"#111"}}>{p.cliente||"—"}</td>
                <td style={TD}>{p.time} {p.uniforme&&`(${p.uniforme})`}</td>
                <td style={TD}>{p.tamanho}</td>
                <td style={{...TD,textAlign:"center"}}>{p.qtd}</td>
                <td style={{...TD,fontWeight:700}}>{p.precoVenda>0?brl(p.precoVenda*p.qtd):"—"}</td>
                <td style={{...TD,color:"#dc2626"}}>{p.custoProduto>0?brl((p.custoProduto+p.custoTaxa)*p.qtd):"—"}</td>
                <td style={{...TD,color:"#9ca3af",maxWidth:100}}>{p.obs||"—"}</td>
                <td style={TD}><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{ST_FORN.filter(x=>x!==p.status).map(x=><Btn key={x} v="sm" onClick={()=>mover(p.id,x)} sx={{fontSize:10,padding:"3px 7px"}}>→ {x.replace("Chegou - ","")}</Btn>)}</div></td>
                <td style={TD}><div style={{display:"flex",gap:4}}><Btn v="sm" onClick={()=>abrirE(p)}>✏</Btn><Btn v="danger" onClick={()=>del(p.id)}>🗑</Btn></div></td>
              </HRow>
            ))}</tbody>
          </table></div>
        </Section>
      ))}
      {db.pedidosFornecedor.length===0&&<Section><Empty msg="Nenhum pedido. Clique em '+ Novo Pedido'." icon="🏭"/></Section>}
      {modal&&<Modal title={modal==="editar"?"Editar Pedido":"Novo Pedido Fornecedor"} onClose={()=>setModal(null)} wide>
        <div style={{display:"flex",flexWrap:"wrap",gap:12}}>
          <Field label="Data" half><Inp type="date" value={f.data} onChange={e=>s("data",e.target.value)}/></Field>
          ...
          <Field label="Cliente (quem pediu)"><Inp value={f.cliente} onChange={e=>s("cliente",e.target.value)} placeholder="Nome (opcional)"/></Field>
          <Field label="Time / Camisa" half><Inp list="lst-tf" value={f.time} onChange={e=>s("time",e.target.value)} placeholder="ex: Bahia"/><datalist id="lst-tf">{times.map(t=><option key={t} value={t}/>)}</datalist></Field>
          <Field label="Uniforme" half><Sel value={f.uniforme} onChange={e=>s("uniforme",e.target.value)}>{UNIFORMES.map(u=><option key={u}>{u}</option>)}</Sel></Field>
          <Field label="Tamanho" third><Sel value={f.tamanho} onChange={e=>s("tamanho",e.target.value)}>{TAMANHOS.map(t=><option key={t}>{t}</option>)}</Sel></Field>
          <Field label="Quantidade" third><Inp type="number" min="1" value={f.qtd} onChange={e=>n("qtd",e.target.value)}/></Field>
          <Field label="Preço Venda (R$)" third><Inp type="number" min="0" step="0.01" value={f.precoVenda} onChange={e=>n("precoVenda",e.target.value)}/></Field>
          <Field label="Custo Produto (R$)" half><Inp type="number" min="0" step="0.01" value={f.custoProduto} onChange={e=>n("custoProduto",e.target.value)}/></Field>
          <Field label="Custo Taxa (R$)" half><Inp type="number" min="0" step="0.01" value={f.custoTaxa} onChange={e=>n("custoTaxa",e.target.value)}/></Field>
          <Field label="Observação"><Inp value={f.obs} onChange={e=>s("obs",e.target.value)} placeholder="ex: urgente, tamanho especial..."/></Field>
        </div>
        <ModalBtns onClose={()=>setModal(null)} onSave={salv}/>
      </Modal>}
    </div>
  );
}
...
export default function App() {
  const [logado,setLogado]=useState(()=>sessionStorage.getItem("t11_ok")==="1");
  ...
  const login=()=>{sessionStorage.setItem("t11_ok","1");setLogado(true);};
  const logout=()=>{sessionStorage.removeItem("t11_ok");setLogado(false);};

  const savePed=d=>{ setDb(prev=>{ if(d.id) return{...prev,pedidos:prev.pedidos.map(p=>p.id===d.id?d:p)}; const id=prev.nextId+1; return{...prev,nextId:id,pedidos:[...prev.pedidos,{...d,id}]}; }); setMP(null); };
  const delPed=id=>{ if(!window.confirm("Excluir pedido?"))return; setDb(prev=>({...prev,pedidos:prev.pedidos.filter(p=>p.id!==id)})); };
  const savePro=d=>{ setDb(prev=>{ if(d.id) return{...prev,produtos:prev.produtos.map(p=>p.id===d.id?d:p)}; const id=prev.nextId+1; return{...prev,nextId:id,produtos:[...prev.produtos,{...d,id}]}; }); setMPr(null); };
  const delPro=id=>{ if(!window.confirm("Excluir produto?"))return; setDb(prev=>({...prev,produtos:prev.produtos.filter(p=>p.id!==id)})); };

  if(!logado) return <Login onLogin={login}/>;

  // Ensure full screen layout — no margins, no cuts
  useEffect(()=>{
    ...
    return () => { const el = document.getElementById("t11-global"); if(el) el.remove(); };
  },[]);

  const tarefasHj=db.tarefas.filter(t=>!t.feita&&t.data===hoje()).length;
  const baixosQt=db.produtos.filter(p=>p.qtd<=p.qtdMin).length;

  const NAVS=[
    {k:"dashboard",l:"Dashboard",    i:"▪", sec:"PRINCIPAL"},
    {k:"estoque",  l:"Estoque",      i:"▪"},
    {k:"pedidos",  l:"Pedidos",      i:"▪"},
    {k:"gestao",   l:"Gestão",       i:"▪"},
    {k:"custo",    l:"Custo / Lucro",i:"▪", sec:"FINANCEIRO"},
    {k:"caixa",    l:"Caixa",        i:"▪"},
    {k:"tarefas",  l:"Tarefas",      i:"▪", sec:"GESTÃO"},
    {k:"fornecedor",l:"Fornecedores",i:"▪"},
  ];
  const titulos={dashboard:"Dashboard",estoque:"Estoque",pedidos:"Pedidos",gestao:"Gestão de Pedidos",custo:"Custo / Lucro",caixa:"Caixa",tarefas:"Tarefas",fornecedor:"Fornecedores"};

  return (
    <div style={{display:"flex",width:"100vw",height:"100vh",fontFamily:"'Inter',-apple-system,BlinkMacSystemFont,sans-serif",background:"#f5f5f5",overflow:"hidden",position:"fixed",top:0,left:0}}>

      {/* ── SIDEBAR ── */}
      <div style={{width:220,minWidth:220,background:"#111",display:"flex",flexDirection:"column",height:"100vh",overflowY:"auto",flexShrink:0}}>
        <div style={{padding:"18px 16px 16px",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:42,height:42,borderRadius:10,background:"linear-gradient(135deg,#1a1a1a 0%,#333 100%)",border:"2px solid rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,boxShadow:"0 4px 12px rgba(0,0,0,0.4)"}}>⚽</div>
            <div>
              <div style={{fontSize:17,fontWeight:900,color:"#fff",letterSpacing:"-0.5px",lineHeight:1.1}}>T11 Sports</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",marginTop:2,letterSpacing:"0.5px",textTransform:"uppercase"}}>Gestão da Loja</div>
            </div>
          </div>
        </div>
        <div style={{flex:1,padding:"8px 6px"}}>
          {NAVS.map(({k,l,i,sec})=>{
            ...
            return (
              <div key={k}>
                {sec&&<div style={{padding:"12px 10px 4px",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.2)",textTransform:"uppercase",letterSpacing:"1.5px"}}>{sec}</div>}
                <button onClick={()=>setPag(k)} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}
                  style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",margin:"1px 0",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:pag===k?700:400,color:pag===k?"#fff":h?"rgba(255,255,255,0.8)":"rgba(255,255,255,0.5)",background:pag===k?"rgba(255,255,255,0.12)":h?"rgba(255,255,255,0.06)":"none",border:pag===k?"1px solid rgba(255,255,255,0.12)":"1px solid transparent",textAlign:"left",width:"100%",boxSizing:"border-box",fontFamily:"inherit",transition:"all 0.15s"}}>
                  <span style={{display:"flex",alignItems:"center",gap:8}}><span>{i}</span>{l}</span>
                  <div style={{display:"flex",gap:4}}>
                    {k==="tarefas"&&tarefasHj>0&&<span style={{background:"#f97316",color:"#fff",fontSize:10,fontWeight:800,padding:"1px 6px",borderRadius:10}}>{tarefasHj}</span>}
                    {k==="estoque"&&baixosQt>0&&<span style={{background:"#dc2626",color:"#fff",fontSize:10,fontWeight:800,padding:"1px 6px",borderRadius:10}}>{baixosQt}</span>}
                  </div>
                </button>
              </div>
            );
        ...
        <div style={{padding:"12px 10px",borderTop:"1px solid rgba(255,255,255,0.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"6px 8px",marginBottom:8}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:900,color:"#fff",flexShrink:0}}>JF</div>
            <div><div style={{fontSize:13,color:"#fff",fontWeight:700}}>Fabrício</div><div style={{fontSize:10,color:"rgba(255,255,255,0.25)"}}>Administrador</div></div>
          </div>
          <button onClick={logout} style={{width:"100%",padding:"8px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:12,fontFamily:"inherit",transition:"all 0.15s"}}>Sair</button>
        </div>
      </div>

      {/* ── MAIN ── */}
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,maxWidth:"100%",height:"100vh",overflow:"hidden"}}>
        <div style={{background:"#fff",borderBottom:"1px solid #e5e7eb",padding:"0 28px",height:56,minHeight:56,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10,boxShadow:"0 1px 3px rgba(0,0,0,0.05)",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:3,height:20,background:"#dc2626",borderRadius:2}}/>
            <div style={{fontWeight:800,fontSize:16,color:"#111"}}>{titulos[pag]}</div>
          </div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            {(pag==="estoque")&&<Btn onClick={()=>setMPr({})}>+ Produto</Btn>}
            {(pag==="pedidos"||pag==="gestao")&&<Btn onClick={()=>setMP({})}>+ Pedido</Btn>}
            <div style={{fontSize:12,color:"#9ca3af"}}>{new Date().toLocaleDateString("pt-BR",{day:"numeric",month:"long",year:"numeric"})}</div>
          </div>
        </div>
        <div style={{flex:1,padding:24,overflowY:"auto",overflowX:"hidden",height:"calc(100vh - 56px)"}}>
          {pag==="dashboard" &&<PageDashboard db={db} setDb={setDb}/>}
          {pag==="estoque"   &&<PageEstoque   db={db} onAdd={()=>setMPr({})} onEdit={p=>setMPr(p)} onDelete={delPro}/>}
          {pag==="pedidos"   &&<PagePedidos   db={db} onAdd={()=>setMP({})}  onEdit={p=>setMP(p)}  onDelete={delPed} onUpdateMeta={mt=>setDb(prev=>({...prev,meta:mt}))}/>}
          {pag==="gestao"    &&<PageGestao    db={db} onEdit={p=>setMP(p)}   onAdd={()=>setMP({})}/>}
          {pag==="custo"     &&<PageCusto     db={db} setDb={setDb}/>}
          {pag==="caixa"     &&<PageCaixa     db={db} setDb={setDb}/>}
          {pag==="tarefas"   &&<PageTarefas   db={db} setDb={setDb}/>}
          {pag==="fornecedor"&&<PageFornecedor db={db} setDb={setDb}/>}
        </div>
      </div>

      {/* ── MODAIS GLOBAIS ── */}
      {mP!==null&&<Modal title={mP?.id?"Editar Pedido":"Novo Pedido"} onClose={()=>setMP(null)} wide><FormPedido inicial={mP?.id?mP:null} produtos={db.produtos} onSave={savePed} onClose={()=>setMP(null)}/></Modal>}
      {mPr!==null&&<Modal title={mPr?.id?"Editar Produto":"Novo Produto"} onClose={()=>setMPr(null)} wide><FormProduto inicial={mPr?.id?mPr:null} onSave={savePro} onClose={()=>setMPr(null)}/></Modal>}
    </div>
  );
}
